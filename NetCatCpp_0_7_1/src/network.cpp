...
bool netcat_resolvehost(nc_host_t* dst, const char* name)
{
	...
	{ /* couldn't translate: it must be a name! */
		...
		if (!opt_debug && (opt_verbose < 1)) return TRUE;

		/* do inverse lookups in a separated loop for each collected addresses */
		for (i = 0; dst->iaddrs[i].s_addr && (i < MAXINETADDRS); i++)
		{
			hostent = gethostbyaddr((char*)&dst->iaddrs[i], sizeof(dst->iaddrs[0]), AF_INET);

			if (!hostent || !hostent->h_name)
			{
				ncprint(NCPRINT_VERB1 | NCPRINT_WARNING, _("Inverse name lookup failed for `%s'"), dst->addrs[i]);
				continue;
			}
			...
			if (strcasecmp(dst->name, hostent->h_name))
			{
				...
				if (!hostent) continue; /* FIXME: missing information analysis */

				for (xcmp = 0; hostent->h_addr_list[xcmp] && (xcmp < MAXINETADDRS); xcmp++)
				{
					if (!memcmp(&dst->iaddrs[i], hostent->h_addr_list[xcmp], sizeof(dst->iaddrs[0]))) goto found_real_host;
				}

				ncprint(NCPRINT_WARNING | NCPRINT_VERB1, _("This host's reverse DNS doesn't match! %s -- %s"), hostent->h_name, dst->name);
				continue;

			found_real_host:
				ncprint(NCPRINT_NOTICE | NCPRINT_VERB2, _("Real hostname for %s [%s] is %s"), dst->name, dst->addrs[i], savedhost);
				continue;
			}
			else if (!host_auth_taken)
			{ /* case: take only the first one as auth */
	...
	{ /* `name' is a numeric address, try reverse lookup */
		...
		if (!hostent)
			ncprint(NCPRINT_VERB2 | NCPRINT_WARNING, _("Inverse name lookup failed for `%s'"), name);
		else
		{
			...
			{
				ncprint(NCPRINT_VERB1 | NCPRINT_WARNING, _("Host %s isn't authoritative! (direct lookup failed)"), dst->addrs[0]);
				goto check_failed;
			}
			for (i = 0; hostent->h_addr_list[i] && (i < MAXINETADDRS); i++)
				if (!memcmp(&dst->iaddrs[0], hostent->h_addr_list[i], sizeof(dst->iaddrs[0]))) return TRUE;

			ncprint(NCPRINT_VERB1 | NCPRINT_WARNING, _("Host %s isn't authoritative! (direct lookup mismatch)"), dst->addrs[0]);
			ncprint(NCPRINT_VERB1, _("  %s -> %s  BUT  %s -> %s"), dst->addrs[0], dst->name, dst->name, netcat_inet_ntop(hostent->h_addr_list[0]));
...
bool netcat_getport(nc_port_t* dst, const char* port_string, unsigned short port_num)
{
	const char* get_proto = (opt_proto == NETCAT_PROTO_UDP ? "udp" : "tcp");
	struct servent* servent;

	debug_v(("netcat_getport(dst=%p, port_string=\"%s\", port_num=%hu)", (void*)dst, NULL_STR(port_string), port_num));
...
const char* netcat_strid(const nc_host_t* host, const nc_port_t* port)
{
	...
	if (port->name[0]) p += snprintf(p, sizeof(buf) + buf - p, " (%s)", port->name);
...
int netcat_socket_new_connect(int domain, int type, const struct in_addr* addr, in_port_t port, const struct in_addr* local_addr, in_port_t local_port)
{
	...
	debug_dv(
	    ("netcat_socket_new_connect(addr=%p, port=%hu, local_addr=%p, local_"
	     "port=%hu)",
	     (void*)addr, ntohs(port), (void*)local_addr, ntohs(local_port)));
	...
	if (sock < 0) return sock; /* just forward the error code */
	...
	if (local_addr || local_port)
	{
		...
		if (local_addr) memcpy(&my_addr.sin_addr, local_addr, sizeof(my_addr.sin_addr));
	...
	if ((ret = fcntl(sock, F_GETFL, 0)) >= 0) ret = fcntl(sock, F_SETFL, ret | O_NONBLOCK);
	if (ret < 0)
	{
		ret = -4;
		goto err;
...
int netcat_socket_new_listen(int domain, const struct in_addr* addr, in_port_t port)
{
...
call_select:
	...
	if (ret < 0)
	{
		...
		if (errno == EINTR) goto call_select;
