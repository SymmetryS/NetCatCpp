...
#define TELNET_DM                                                  \
	242                /* (Data Mark) The data stream portion of a \
...
#define TELNET_WILL                                     \
	251 /* Indicates the desire to begin performing,    \
	     * or confirmation that you are now performing, \
	     * the indicated option. */
#define TELNET_WONT                                \
	252 /* Indicates the refusal to perform, or to \
	     * continue performing, the indicated option. */
#define TELNET_DO                                     \
	253 /* Indicates the request that the other party \
	     * perform, or confirmation that you are      \
	     * expecting the other party to perform, the  \
	     * indicated option. */
#define TELNET_DONT                                                 \
	254                /* Indicates the demand that the other party \
...
void netcat_telnet_parse(nc_sock_t* ncsock)
{
	...
	for (i = 0; i < ref_size; i++)
	{
		...
		if ((buf[i] != TELNET_IAC) && (l == 0)) continue;
