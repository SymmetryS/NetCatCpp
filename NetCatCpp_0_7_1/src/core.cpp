...
static int core_udp_listen(nc_sock_t* ncsock)
{
	...
	if (ncsock->local_host.iaddrs[0].s_addr) need_udphelper = FALSE;
#endif
	...
	if (!need_udphelper)
		ncprint(NCPRINT_VERB2, _("Listening on %s"), netcat_strid(&ncsock->local_host, &ncsock->local_port));
	else
		ncprint(NCPRINT_VERB2, _("Listening on %s (using %d sockets)"), netcat_strid(&ncsock->local_host, &ncsock->local_port), sockbuf[0]);
	...
	while (TRUE)
	{
		...
		for (socks_loop = 1; socks_loop <= sockbuf[0]; socks_loop++)
		{
			...
			debug_v(("received packet from %s:%d%s", netcat_inet_ntop(&rem_addr.sin_addr), ntohs(rem_addr.sin_port), (opt_zero ? "" : ", using as default dest")));
			...
			if (ret == 0)
			{
				...
				ncprint(NCPRINT_VERB1, _("Received packet from %s:%d -> %s:%d (local)"), tmpbuf, ntohs(rem_addr.sin_port), netcat_inet_ntop(&local_addr.sin_addr), ntohs(local_addr.sin_port));
			}
			else
				ncprint(NCPRINT_VERB1, _("Received packet from %s:%d"), netcat_inet_ntop(&rem_addr.sin_addr), ntohs(rem_addr.sin_port));

			if (opt_zero)
			{ /* output the packet right here right now */
				...
				if (opt_hexdump)
				{
					...
					fprintf(output_fp, "Received %d bytes from %s:%d\n", recv_ret, netcat_inet_ntop(&rem_addr.sin_addr), ntohs(rem_addr.sin_port));
#endif
...
static int core_tcp_connect(nc_sock_t* ncsock)
{
	...
	sock = netcat_socket_new_connect(PF_INET, SOCK_STREAM, &ncsock->host.iaddrs[0], ncsock->port.netnum, (ncsock->local_host.iaddrs[0].s_addr ? &ncsock->local_host.iaddrs[0] : NULL),
	                                 ncsock->local_port.netnum);

	if (sock < 0) ncprint(NCPRINT_ERROR | NCPRINT_EXIT, "Couldn't create connection (err=%d): %s", sock, strerror(errno));
	...
	if (ret > 0)
	{
		...
		if (ret < 0) ncprint(NCPRINT_ERROR | NCPRINT_EXIT, "Critical system request failed: %s", strerror(errno));
	...
	else if (ret)
	{
		...
		ncprint(NCPRINT_ERROR | NCPRINT_EXIT, "Critical system request failed: %s", strerror(errno));
	}
...
static int core_tcp_listen(nc_sock_t* ncsock)
{
	...
	sock_listen = netcat_socket_new_listen(PF_INET, &ncsock->local_host.iaddrs[0], ncsock->local_port.netnum);
	if (sock_listen < 0) ncprint(NCPRINT_ERROR | NCPRINT_EXIT, _("Couldn't setup listening socket (err=%d)"), sock_listen);
	...
	while (TRUE)
	{
		...
		if ((ncsock->host.iaddrs[0].s_addr && memcmp(&ncsock->host.iaddrs[0], &my_addr.sin_addr, sizeof(ncsock->host.iaddrs[0]))) || (netcat_flag_count() && !netcat_flag_get(ntohs(my_addr.sin_port))))
		{
			ncprint(NCPRINT_VERB2, _("Unwanted connection from %s:%hu (refused)"), netcat_inet_ntop(&my_addr.sin_addr), ntohs(my_addr.sin_port));
			goto refuse;
		}
		ncprint(NCPRINT_VERB1, _("Connection from %s:%hu"), netcat_inet_ntop(&my_addr.sin_addr), ntohs(my_addr.sin_port));

		/* with zero I/O mode we don't really accept any connection */
		if (opt_zero) goto refuse;
...
/*
    ...
    $ ./nc_cpp.out -l -t -p 8080 -e /usr/bin/bash	// Server
    $ ./nc_cpp.out -n -t 10.3.0.201 8080			// Client
    ...
    *nc_main = nc_sock_t
                {
                    ...
                    .timeout = 0,
                    .proto = PROTO_TCP,
                    .local_host = nc_host_t{ .name = "", .addrs = [], .iaddrs = [] },
                    .host = nc_host_t{ .name = "", .addrs = ["10.3.0.201"], .iaddrs = ["3372221194"] },
                    .local_port = nc_port_t{ .name = "", .ascnum = "", .num = 0, .netnum = 0 },
                    .port = nc_port_t{ .name = "webcache", .ascnum = "8080", .num = 8080, .netnum = 36895 },
                    .sendq = nc_buffer_t{ .head = 0, .pos = 0, .len = 0 },
                    .recvq = nc_buffer_t{ .head = 0, .pos = 0, .len = 0 }
                }
    *nc_slave = nc_sock_t
                {
                    .fd = 0,
                    ...
                    .proto = PROTO_UNSPEC,
                    .local_host = nc_host_t{ .name = "", .addrs = [], .iaddrs = [] },
                    .host = nc_host_t{ .name = "", .addrs = [], .iaddrs = [] },
                    .local_port = nc_port_t{ .name = "", .ascnum = "", .num = 0, .netnum = 0 },
                    .port = nc_port_t{ .name = "", .ascnum = "", .num = 0, .netnum = 0 },
                    .sendq = nc_buffer_t{ .head = 0, .pos = 0, .len = 0 },
                    .recvq = nc_buffer_t{ .head = 0, .pos = 0, .len = 0 }
                }
*/
...
int core_readwrite(nc_sock_t* nc_main, nc_sock_t* nc_slave)
{
	...
	debug_v(("core_readwrite(nc_main=%p, nc_slave=%p)", (void*)nc_main, (void*)nc_slave));
	std::cout << "*nc_main = " << *nc_main << std::endl;
	std::cout << "*nc_slave = " << *nc_slave << std::endl;

	// set the actual input and output fds and find out the max fd + 1.
	int fd_sock = nc_main->fd;
	assert(fd_sock >= 0);
	...
	int fd_max = 1 + (fd_stdin > fd_sock ? fd_stdin : fd_sock);
	struct timeval delayer;
	delayer.tv_sec = 0;
	delayer.tv_usec = 0;

	signal_handler = FALSE;  // use the internal signal handler.
	...
	while (in_loop)
	{
		...
		{
			/* FIXME: call_select = false but could call it anyway and one of them could be set.. so what happens? */
			debug_v(("watching slave sock for incoming data (recvq is empty)"));
			if (use_stdin || (netcat_mode == NETCAT_TUNNEL)) FD_SET(fd_stdin, &ins);
		}
		else
		{
			call_select = FALSE;
		...
		if (call_select || delayer.tv_sec || delayer.tv_usec)
		{
			...
			int ret = select(fd_max, &ins, &outs, NULL, (delayer.tv_sec || delayer.tv_usec ? &delayer : NULL));
			...
			if (ret < 0)
			{
				/* something went wrong (maybe a legal signal) */
				if (errno == EINTR) goto handle_signal;
		...
		if (call_select && FD_ISSET(fd_stdin, &ins))
		{
			...
			{
				// we can overwrite safely since if the receive queue is busy this fd is not watched at all.
		...
		if (nc_main->sendq.len > 0)
		{
			...
			if (opt_interval)
			{
				...
				if (delayer.tv_sec || delayer.tv_usec) goto skip_sect; /* the delay is not yet over! */
			...
			{
				debug_v(("Damn! I wanted to send to sock %d bytes but it only sent %d", data_len, write_ret));
				data_len = write_ret;
			}
		...
		if (call_select && FD_ISSET(fd_sock, &ins))
		{
			if ((nc_main->proto == NETCAT_PROTO_UDP) && opt_zero)
			{
				...
				read_ret = recvfrom(fd_sock, buf, sizeof(buf), 0, (struct sockaddr*)&recv_addr, &recv_len);
				/* when recvfrom() call fails, recv_addr remains untouched */
				debug_dv(("recvfrom(net) = %d (address=%s:%d)", read_ret, netcat_inet_ntop(&recv_addr.sin_addr), ntohs(recv_addr.sin_port)));
			}
			else
			{
		...
		if (nc_slave->sendq.len > 0)
		{
			...
			if (write_ret < data_len)
			{
				debug_v(("Damn! I wanted to send to stdout %d bytes but it only sent %d", data_len, write_ret));
				data_len = write_ret;
			}
...
#ifndef USE_OLD_HEXDUMP
				if ((nc_main->proto == NETCAT_PROTO_UDP) && opt_zero)
					fprintf(output_fp, "Received %d bytes from %s:%d\n", write_ret, netcat_inet_ntop(&recv_addr.sin_addr), ntohs(recv_addr.sin_port));
				else
