...
void ncprint(int type, const char* fmt, ...)
{
	...
	if (!opt_debug)
	{
		if ((flags & NCPRINT_VERB2) && (opt_verbose < 2)) goto end;

		if ((flags & NCPRINT_VERB1) && (opt_verbose < 1)) goto end;
	}

	/* known flags */
	if (flags & NCPRINT_STDOUT) fstream = stdout;
	if (flags & NCPRINT_NONEWLINE) newline = 0;
...
void netcat_printstats(bool force)
{
	...
	if ((bytes_recv > 0) && !isdigit((int)*p)) snprintf(++p, sizeof(str_recv) - 32, " (%lu)", bytes_recv);

	netcat_snprintnum(str_sent, 32, bytes_sent);
	assert(str_sent[0]);
	for (p = str_sent; *(p + 1); p++); /* find the last char */
	if ((bytes_sent > 0) && !isdigit((int)*p)) snprintf(++p, sizeof(str_sent) - 32, " (%lu)", bytes_sent);

	ncprint(NCPRINT_NONEWLINE | (force ? 0 : NCPRINT_VERB2), _("Total received bytes: %s\nTotal sent bytes: %s\n"), str_recv, str_sent);
}
...
char* netcat_string_split(char** buf)
{
	...
	if (!buf || (*buf == NULL)) return *buf = (char*)"";
...
void netcat_printhelp(char* argv0)
{
	...
	printf("\n");
	printf(
	    _("Remote port number can also be specified as range.  "
	      "Example: '1-1024'\n"));
	printf("\n");
}
...
void netcat_printversion(void)
{
	...
	printf(
	    _("Copyright (C) 2002 - 2003  Giovanni Giacobbi\n\n"
...
void update_timeval(struct timeval* target)
{
	...
	if (target == NULL)
	{                                                  /* just initialize the seed */
		if (gettimeofday(&dd_start, &dd_zone)) return; /* can't handle this type of error */
	}
	else
	{
		if (gettimeofday(&dd_end, &dd_zone)) return; /* can't handle this type of error */
