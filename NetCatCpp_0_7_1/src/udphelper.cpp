...
int udphelper_ancillary_read(struct msghdr* my_hdr, struct sockaddr_in* get_addr)
{
	...
	if (my_hdr->msg_controllen > 0)
	{
		...
		for (get_cmsg = CMSG_FIRSTHDR(my_hdr); get_cmsg; get_cmsg = CMSG_NXTHDR(my_hdr, get_cmsg))
		{
...
int udphelper_sockets_open(int** sockbuf, in_port_t nport)
{
	...
	if (dummy_sock < 0) goto err;

	/* find out the interfaces configuration, allocating more memory if
	   necessary. */
	do
	{ /* FIXME: set max buffer size (what is max IF num?) */
		...
		nc_ifreq = (struct lifreq*)malloc(alloc_size);
	...
	while (if_pos < nc_ifconf.lifc_len)
	{
		...
		if (nc_ifreq->lifr_addr.ss_family != AF_INET) continue;
		...
		if (!(nc_ifreq->lifr_flags & IFF_UP)) continue;
		...
		if (newsock > my_sockbuf_max) my_sockbuf_max = newsock;
		...
		if (nport == 0)
		{
			...
			ret = getsockname(newsock, (struct sockaddr*)&if_addr, (socklen_t*)&sa_tmp_len);
			if (ret < 0) goto err;
	...
	*sockbuf = my_sockbuf;
	...
	if (my_sockbuf[0] > 0) return my_sockbuf_max;
...
err:
	...
	for (i = 1; my_sockbuf && (i <= sock_total); i++) close(my_sockbuf[i]);
	free(my_sockbuf);
	*sockbuf = NULL;
...
void udphelper_sockets_close(int* sockbuf)
{
	...
	for (i = 1; i <= sockbuf[0]; i++)
		if (sockbuf[i] >= 0) close(sockbuf[i]);
