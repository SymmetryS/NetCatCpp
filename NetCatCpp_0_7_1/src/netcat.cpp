...
/* nc_host_t **gates = nullptr; */     /* LSRR hop hostpoop */
/* char *optbuf = nullptr; */          /* LSRR or sockopts */
FILE* output_fp = nullptr;             /* output fd (FIXME: i don't like this) */
...
nc_mode_t netcat_mode = (nc_mode_t)0;    /* Netcat working modality */
...
char* opt_outputfile = nullptr;          /* hexdump output file */
char* opt_exec = nullptr;                /* program to exec after connecting */
...
static void got_term(int z)
{
	...
	debug_v(("_____ RECEIVED SIGTERM _____ [signal_handler=%s]", BOOL_TO_STR(signal_handler)));
	got_sigterm = TRUE;
...
static void got_int(int z)
{
	...
	debug_v(("_____ RECEIVED SIGINT _____ [signal_handler=%s]", BOOL_TO_STR(signal_handler)));
	got_sigint = TRUE;
	if (signal_handler)
	{                                 /* default action */
...
{
	debug_dv(("_____ RECEIVED SIGUSR1 _____ [signal_handler=%s]", BOOL_TO_STR(signal_handler)));
	if (signal_handler) /* default action */
	{
...
static void ncexec(nc_sock_t* ncsock)
{
	...
	int saved_stderr = dup(STDERR_FILENO);  // save the stderr fd because we may need it later

	// duplicate the socket for the child program
	dup2(ncsock->fd, STDIN_FILENO);     // the precise order of fiddlage  		// 重定向标准输入到套接字
	close(ncsock->fd);                  // is apparently crucial; this is
	dup2(STDIN_FILENO, STDOUT_FILENO);  // swiped directly out of "inetd". 		// 重定向标准输出到套接字
	dup2(STDIN_FILENO, STDERR_FILENO);  // also duplicate the stderr channel 	// 重定向标准错误到套接字

	// change the label for the executed program
	char* program = nullptr;
	if ((program = strrchr(opt_exec, '/')))
	{
		++program;  // shorter argv[0]
	}
	else
	{
		program = opt_exec;
	...
	execl("/bin/sh", program, "-c", opt_exec, nullptr);
#else
	execl(opt_exec, program, nullptr);
#endif
	dup2(saved_stderr, STDERR_FILENO);
	ncprint(NCPRINT_ERROR | NCPRINT_EXIT, _("Couldn't execute %s: %s"), opt_exec, strerror(errno));
} /* end of ncexec() */
...
static const struct option long_options[] = 
{
	...
	{"pointer", required_argument, nullptr, 'G'},
	{"help", no_argument, nullptr, 'h'},
	{"interval", required_argument, nullptr, 'i'},
	{"listen", no_argument, nullptr, 'l'},
	{"tunnel", required_argument, nullptr, 'L'},
	{"dont-resolve", no_argument, nullptr, 'n'},
	{"output", required_argument, nullptr, 'o'},
	{"local-port", required_argument, nullptr, 'p'},
	{"tunnel-port", required_argument, nullptr, 'P'},
	{"randomize", no_argument, nullptr, 'r'},
	{"source", required_argument, nullptr, 's'},
	{"tunnel-source", required_argument, nullptr, 'S'},
#ifndef USE_OLD_COMPAT
	{"tcp", no_argument, nullptr, 't'},
	{"telnet", no_argument, nullptr, 'T'},
#else
	{"tcp", no_argument, nullptr, 1},
	{"telnet", no_argument, nullptr, 't'},
#endif
	{"udp", no_argument, nullptr, 'u'},
	{"verbose", no_argument, nullptr, 'v'},
	{"version", no_argument, nullptr, 'V'},
	{"hexdump", no_argument, nullptr, 'x'},
...
{
	printf("getpid() = %d\n", getpid());
	int total_ports, left_ports, connect_ret = -1, glob_ret = EXIT_FAILURE;
	...
	while (TRUE)
	{
		...
		switch (c)
		{
			...
			case 'i': /* line/ports interval time (seconds) */
				...
				if (opt_interval <= 0) ncprint(NCPRINT_ERROR | NCPRINT_EXIT, _("Invalid interval time \"%s\""), optarg);
				break;
			case 'l': /* mode flag: listen mode */
				if (netcat_mode != NETCAT_UNSPEC)
				{
					ncprint(NCPRINT_ERROR | NCPRINT_EXIT, _("You can specify mode flags (`-l' and `-L') only once"));
				}
				netcat_mode = NETCAT_LISTEN;
				break;
			case 'L': /* mode flag: tunnel mode */
				if (netcat_mode != NETCAT_UNSPEC) ncprint(NCPRINT_ERROR | NCPRINT_EXIT, _("You can specify mode flags (`-l' and `-L') only once"));
				if (opt_zero) ncprint(NCPRINT_ERROR | NCPRINT_EXIT, _("`-L' and `-z' options are incompatible"));
				do
				{
					...
					if (div && *(div + 1))
						*div++ = '\0';
						...
						ncprint(NCPRINT_ERROR | NCPRINT_EXIT, _("Invalid target string for `-L' option"));

					/* lookup the remote address and the remote port for tunneling */
					if (!netcat_resolvehost(&connect_sock.host, optarg)) ncprint(NCPRINT_ERROR | NCPRINT_EXIT, _("Couldn't resolve tunnel target host: %s"), optarg);
					if (!netcat_getport(&connect_sock.port, div, 0)) ncprint(NCPRINT_ERROR | NCPRINT_EXIT, _("Invalid tunnel target port: %s"), div);
			...
			case 'p': /* local source port */
				...
				{
					ncprint(NCPRINT_ERROR | NCPRINT_EXIT, _("Invalid local port: %s"), optarg);
				}
				break;
			case 'P': /* used only in tunnel mode (source port) */
				if (!netcat_getport(&connect_sock.local_port, optarg, 0))
				{
					ncprint(NCPRINT_ERROR | NCPRINT_EXIT, _("Invalid tunnel connect port: %s"), optarg);
				}
				break;
			...
			case 's': /* local source address */
				...
				if (!netcat_resolvehost(&local_host, optarg)) ncprint(NCPRINT_ERROR | NCPRINT_EXIT, _("Couldn't resolve local host: %s"), optarg);
				break;
			case 'S': /* used only in tunnel mode (source ip) */
				if (!netcat_resolvehost(&connect_sock.local_host, optarg)) ncprint(NCPRINT_ERROR | NCPRINT_EXIT, _("Couldn't resolve tunnel local host: %s"), optarg);
				break;
			...
			case 'w': /* wait time (in seconds) */
				...
				if (opt_wait <= 0) ncprint(NCPRINT_ERROR | NCPRINT_EXIT, _("Invalid wait-time: %s"), optarg);
				break;
			...
			case 'z': /* little or no data xfer */
				...
				{
					ncprint(NCPRINT_ERROR | NCPRINT_EXIT, _("`-L' and `-z' options are incompatible"));
				}
				opt_zero = TRUE;
				break;
			default:
	...
	{
		ncprint(NCPRINT_ERROR | NCPRINT_EXIT, _("`-e' and `-z' options are incompatible"));
	}
	...
	if (opt_debug)
	{
		ncprint(NCPRINT_WARNING, _("Debugging support not compiled, option `-d' discarded. Using maximum verbosity."));
	}
#endif
	...
	if (opt_random)
	{
		...
		SRAND(time(0));
#else
		ncprint(NCPRINT_WARNING, _("Randomization support not compiled, option `-r' discarded."));
#endif
	}
	...
	if (opt_outputfile)
	{
		...
		if (!output_fp) ncprint(NCPRINT_ERROR | NCPRINT_EXIT, _("Failed to open output file: %s"), strerror(errno));
	}
	else
	{
		output_fp = stderr;
	...
	if (optind < argc)
	{
		...
		{
			ncprint(NCPRINT_ERROR | NCPRINT_EXIT, _("Couldn't resolve host \"%s\""), myhost);
		}
	}
	else
	{
	...
	while (optind < argc)
	{
		...
		{
			/* could be in the forms: N1-N2, -N2, N1- */
			*q++ = 0;
			if (*parse)
			{
				if (netcat_getport(&port_tmp, parse, 0))
	...
	if (netcat_mode > NETCAT_CONNECT)
	{
		...
		int accept_ret = core_listen(&listen_sock);
		...
		if (accept_ret < 0)
		{
			...
			ncprint(NCPRINT_VERB1 | NCPRINT_EXIT, _("Listen mode failed: %s"), strerror(errno));
		}
		...
		else
		{
			...
			if (connect_ret < 0)
			{
				...
				ncprint(NCPRINT_VERB1, "%s: %s", netcat_strid(&connect_sock.host, &connect_sock.port), strerror(errno));
			}
			else
			{
	...
	if (total_ports == 0) ncprint(NCPRINT_ERROR | NCPRINT_EXIT, _("No ports specified for connection"));
	...
	while (left_ports > 0)
	{
		...
		netcat_getport(&connect_sock.port, nullptr, c);
		...
		if (connect_ret < 0)
		{
			...
			if (total_ports > 1) ncprint_flags = NCPRINT_VERB2;

			ncprint(ncprint_flags, "%s: %s", netcat_strid(&connect_sock.host, &connect_sock.port), strerror(errno));
